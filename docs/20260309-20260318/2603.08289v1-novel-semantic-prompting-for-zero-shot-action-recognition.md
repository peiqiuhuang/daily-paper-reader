...
tags: ["query:bopl"]
score: 7.0
evidence: 用于零样本识别的结构化语义提示
tldr: 本研究针对零样本动作识别（ZSAR）提出SP-CLIP框架。不同于以往侧重时序建模的方法，该研究强调语义提示的重要性。SP-CLIP在不修改视觉编码器或增加参数的情况下，通过引入意图、运动和物体交互等多维度结构化语义描述，增强了冻结视觉语言模型的表达能力。实验证明，该方法在处理细粒度和组合动作时表现优异，在保持高效性的同时显著提升了模型的泛化性能。
selection_source: fresh_fetch
motivation: 现有零样本动作识别方法多关注时序建模或架构调整，忽视了语义提示在理解复杂动作中的巨大潜力。
method: 提出SP-CLIP框架，利用意图、动作和物体交互等多层级结构化语义提示，通过提示聚合与一致性评分对齐视频与文本表征。
result: 在多个标准基准测试中显著提升了识别准确率，尤其在细粒度和组合动作识别任务上表现出极强的竞争力。
conclusion: 语义提示是零样本动作理解的关键信号，能在不改变模型架构的情况下有效增强预训练视觉语言模型的泛化能力。
---
