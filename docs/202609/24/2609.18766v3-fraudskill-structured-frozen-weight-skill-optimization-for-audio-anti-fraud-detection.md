...
date: 2026-09-16
pdf: "https://arxiv.org/pdf/2609.18766v3"
tags: ["query:bopl"]
score: 6.0
evidence: 结构化冻结权重技能优化与基于提示的方法
tldr: 针对音频反欺诈检测中大模型难以适应多变欺诈模式和决策协议的问题，本文提出了FRAUDSkill框架。该方法通过冻结底层音频语言模型，利用外部技能程序、路由策略和决策规则进行优化，实现了结构化的输出控制与多路径推理。在TeleAntiFraud基准测试中，该方法显著提升了检测性能并大幅降低了无效输出，为音频反欺诈提供了一种高效且灵活的适配方案。
selection_source: fresh_fetch
motivation: 现有微调和提示工程方法难以应对不断演变的欺诈模式及复杂的结构化决策协议。
method: 提出FRAUDSkill框架，在保持底层模型冻结的前提下，通过优化外部技能层、路由策略和决策规则来实现结构化控制。
result: "在TeleAntiFraud基准上达到73.50%的Macro-F1，较基线提升31.96%，并将无效输出率降至1.94%。"
conclusion: 外部技能优化是一种无需修改底层模型即可实现高效、可适配结构化音频反欺诈检测的有效方案。
---
