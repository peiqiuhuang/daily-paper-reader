...
title_zh: VisionCoach：通过视觉感知提示增强定位式视频推理
...
## 摘要
视频推理要求模型在帧间定位并追踪与问题相关的证据。虽然带有可验证奖励的强化学习（RL）提高了准确性，但在推理过程中实现可靠的时空定位（spatio-temporal grounding）仍然具有挑战性。此外，提高定位能力通常依赖于大规模训练数据或推理时的感知工具，这增加了标注成本或计算开销。为应对这一挑战，我们提出了 VisionCoach，这是一个输入自适应的强化学习框架，通过在训练阶段引入视觉提示（visual prompting）作为引导，来提升时空定位能力。在强化学习训练期间，视觉提示被选择性地应用于具有挑战性的输入，以放大与问题相关的证据并抑制干扰因素。随后，模型通过自蒸馏（self-distillation）内化这些改进，使其在推理阶段无需视觉提示即可直接在原始视频上进行定位式推理。VisionCoach 由两个组件组成：(1) 视觉提示选择器（Visual Prompt Selector），根据视频和问题预测合适的提示类型；(2) 时空推理器（Spatio-Temporal Reasoner），在视觉提示引导和物体感知定位奖励下通过强化学习进行优化，该奖励强制执行物体身份一致性和多区域边界框重叠。大量实验表明，VisionCoach 在同等设置下，在多种视频推理、视频理解和时间定位基准测试（V-STAR、VideoMME、World-Sense、VideoMMMU、PerceptionTest 和 Charades-STA）中均达到了最先进的性能，同时保持了无需外部工具的单一高效推理路径。我们的结果表明，训练期间的视觉提示提升了定位式视频推理能力，而自蒸馏使模型能够在推理时无需提示即可内化这种能力。
