...
method: 引入轻量级适配器，通过强化学习在序列级和标记级动态调整采样策略，以优化推理资源分配。
result: "在MATH基准上，标记级适配器在固定预算下将Pass@1准确率提升了高达10.2%，显著优于静态基准。"
conclusion: 实验证明，通过序列级和标记级的自适应解码，可以在不微调原模型的情况下有效提升LLM在受限资源下的推理性能。
---
