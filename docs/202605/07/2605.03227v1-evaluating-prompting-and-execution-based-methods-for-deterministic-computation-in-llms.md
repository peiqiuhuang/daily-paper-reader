...
tldr: 本研究系统评估了LLM在确定性计算任务（如二进制计数、算术运算）中的表现。通过对比CoT、Least-to-Most、PoT和SC等提示策略，并引入合成数据集，发现传统提示法准确率有限，而基于代码执行的PoT表现最佳。研究还证明了训练小型专用模型能以极低成本实现完美准确率，强调了外部工具在确定性任务中的必要性。
selection_source: fresh_fetch
motivation: 旨在探究大语言模型在处理需要精确、无误的确定性符号计算任务时的真实能力与局限性。
method: 系统评估了多种提示策略（CoT、Least-to-Most、PoT、SC），并引入了包含多样化自然语言指令的合成数据集进行受控评估。
result: 传统提示法准确率中等，CoT改进有限，而PoT通过生成可执行代码并调用外部解释器实现了完美的准确率。
conclusion: LLM更倾向于模拟推理模式而非执行精确符号计算，结合外部工具或使用专用模型是处理确定性任务更可靠高效的方案。
---
