...
tldr: 随着生成式AI服务的扩展，如何为用户提示词选择合适的模型变得至关重要。现有方法多侧重于保真度（如CLIP-Score），忽视了生成结果的多样性。本文提出DAK-UCB算法，这是一种基于上下文老虎机的在线模型选择框架，通过结合保真度与基于核函数的提示词感知多样性指标，实现了在保持生成质量的同时提升输出多样性的目标。
selection_source: fresh_fetch
motivation: 现有的生成模型选择方法仅关注保真度评分，导致生成的响应可能缺乏多样性。
method: 提出DAK-UCB算法，利用上下文老虎机框架并结合联合核距离与核熵指标，在在线选择过程中平衡保真度与多样性。
result: 实验证明DAK-UCB在处理一系列提示词时，能有效提升模型选择的多样性，同时不损失生成结果的保真度。
