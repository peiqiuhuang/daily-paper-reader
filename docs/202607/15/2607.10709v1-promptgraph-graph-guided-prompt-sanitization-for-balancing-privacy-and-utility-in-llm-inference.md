...
title_zh: PromptGraph：在大语言模型推理中平衡隐私与效用的图引导提示词脱敏方法
...
method: 将提示词建模为属性图，以节点表示片段隐私得分，以边表示片段间的上下文依赖，通过图优化算法选择最佳脱敏集合。
result: 实验表明，PromptGraph 在隐私保护和模型效用之间达到了比现有基准方法更优的平衡。
conclusion: 显式建模片段间的上下文依赖关系是实现高效、隐私保护的 LLM 推理的关键。
---

## 摘要
大语言模型（LLM）服务带来了根本性的隐私挑战。敏感信息不仅可能从姓名或电话号码等显式标识符中推断出来，还可能从原本无害的文本片段（spans）之间的上下文关联中推断出来。现有的脱敏工具通常为单个片段分配隐私或效用信号，而没有显式建模它们之间的成对关系。在本文中，我们提出了 PromptGraph，一种用于隐私保护型 LLM 推理的图引导提示词脱敏方法。PromptGraph 在片段级别估计隐私泄露，并估计片段对之间与效用相关的上下文依赖关系。它将每个提示词表示为一个属性图，其中节点携带片段级别的隐私分数，边编码了为保持效用所需的上下文依赖关系。脱敏目标是选择一个受保护的片段集合，在最大化隐私增益的同时惩罚上下文依赖关系的损失。这种公式化方法在隐藏上下文证据时显式地平衡了隐私和效用。受保护的片段在本地进行脱敏，返回的占位符仅在通过本地一致性检查后才被还原。我们进行了广泛的实验，结果表明 PromptGraph 在隐私和效用之间取得了比提示词隐私基准方法更理想的平衡。
