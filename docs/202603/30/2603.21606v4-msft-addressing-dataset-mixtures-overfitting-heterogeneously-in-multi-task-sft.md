...
motivation: 传统多任务SFT对所有子数据集采用统一的计算预算，导致学习较快的任务过早过拟合，而学习较慢的任务训练不足。
method: 提出一种迭代式过拟合感知搜索算法，动态识别并排除已过拟合的子数据集，并回退到该任务的最优状态继续训练剩余任务。
result: 在10个基准测试和6个基础模型上，mSFT一致优于4种基准方法，且对超参数不敏感，具有极强的鲁棒性。
conclusion: mSFT为多任务SFT提供了一种实用的过拟合感知方案，能有效平衡不同任务的学习进度，最大化模型在多样化数据混合中的潜力。
---
