...
title_zh: BadLLM-TG：一种由大语言模型触发器生成器驱动的后门防御器
...
## 摘要
后门攻击通过使用触发器操纵输出来损害模型的可靠性。触发器逆向（Trigger inversion）可以通过生成器准确地定位这些触发器，因此对后门防御至关重要。然而，文本的离散性质使得现有的基于噪声的触发器生成器无法应用于自然语言处理（NLP）。为了克服这些局限性，我们利用大语言模型（LLMs）中蕴含的丰富知识，提出了一种由 LLM 触发器生成器驱动的后门防御器，称为 BadLLM-TG。它通过提示驱动的强化学习进行优化，并使用受害模型的反馈损失作为奖励信号。随后，生成的触发器被用于通过对抗训练来缓解后门。实验表明，我们的方法平均将攻击成功率降低了 76.2%，比排名第二的防御器性能高出 13.7。
