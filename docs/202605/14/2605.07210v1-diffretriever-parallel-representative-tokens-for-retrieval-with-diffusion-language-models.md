...
tldr: 本研究针对自回归语言模型在多表征检索中存在的串行生成效率低、效果提升不明显的问题，提出了DiffRetriever。该方法利用扩散语言模型（Diffusion LMs）的并行特性，通过在提示词后添加K个掩码位置，在单次双向前向传播中同时获取多个表征向量。实验表明，DiffRetriever在域内和跨域评估中均显著优于单表征模型及自回归多表征基准，在BEIR-7榜单上超越了PromptReps和RepLLaMA等强基准，证明了并行多表征在检索任务中的优越性。
selection_source: fresh_fetch
motivation: 传统自回归模型在生成多表征向量时受限于串行解码，导致检索效率低下且性能提升有限。
method: 提出DiffRetriever，利用扩散模型的双向并行能力，在单次前向传播中同时提取K个代表性Token作为检索向量。
result: 在多个扩散模型骨干上，DiffRetriever的性能均显著优于单Token版本，且在BEIR-7检索任务中达到了领先水平。
conclusion: 扩散语言模型的并行生成机制解决了多表征检索的瓶颈，是构建高效、高性能检索器的有效路径。
---

## 摘要
PromptReps 表明，通过提示自回归语言模型生成查询或段落的稠密和稀疏表示，可以将其直接用作检索器。将此扩展到多个代表性标记对于自回归模型来说效率低下，因为标记必须顺序生成，且先前的多标记变体在性能上并未能可靠地优于单标记解码。我们证明瓶颈在于顺序生成，而非多标记理念本身。DiffRetriever 是一种针对扩散语言模型的代表性标记检索器：它在提示词后附加 K 个掩码位置，并在单次双向前向传递中读取所有 K 个位置。在域内和域外评估中，多标记 DiffRetriever 在我们测试的每个扩散骨干网络上都显著优于单标记版本，而自回归多标记版本的表现则持平或下降，且其延迟成本随 K 增加，而扩散模型则不然。经过有监督微调后，基于 Dream 的 DiffRetriever 是我们对比中最强的 BEIR-7 检索器，领先于 PromptReps、在相同扩散骨干网络上的编码器风格 DiffEmbed 基准，以及经过对比微调的单向量 RepLLaMA。在冻结的基础模型上，针对每个查询的 oracle 表现超过了在相同固定预算下的对比微调，这表明自适应预算选择是未来的研究方向。代码可在 https://github.com/ielab/diffretriever 获取。
