...
tldr: 针对视觉语言模型在少样本微调中易发生灾难性遗忘的问题，本文提出EvoPrompt框架。该方法通过模态共享提示投影器生成层级提示，并采用一种进化训练策略将参数更新解耦为方向和幅度，从而在保留预训练语义方向的同时优化模型。结合特征几何正则化，EvoPrompt在提升下游任务性能的同时，有效兼顾了模型的零样本泛化能力。
selection_source: fresh_fetch
motivation: 旨在解决视觉语言模型在进行参数高效提示学习时，因预训练知识流失而导致的灾难性遗忘挑战。
method: 采用模态共享提示投影器生成层级提示，并通过解耦方向与幅度的进化策略及特征几何正则化来引导提示的稳定演进。
result: 在多个少样本学习基准测试中取得了最先进的性能，并成功保留了预训练模型的原始零样本识别能力。
conclusion: 显式控制提示的进化路径是实现视觉语言模型无遗忘适配的关键，为平衡模型迁移与知识保留提供了新思路。
---
