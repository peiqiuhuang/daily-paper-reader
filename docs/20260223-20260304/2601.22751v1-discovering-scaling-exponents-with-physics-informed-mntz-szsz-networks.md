...
motivation: 传统神经网络在处理具有奇点或临界点的物理系统时，无法显式提取控制系统的幂律缩放指数，缺乏物理可解释性。
method: 引入 MSN-PINN 网络，将缩放指数设为可训练参数，并利用物理信息约束和 Müntz-Szász 基函数进行建模与训练。
result: "实验表明该模型能以极高精度（如 0.009% 误差）恢复奇点指数，且在噪声和稀疏采样环境下具有很强的鲁棒性。"
conclusion: MSN-PINN 成功融合了神经网络的灵活性与渐近分析的可解释性，为自动发现物理系统中的缩放规律提供了有效的新工具。
---

## 摘要
物理系统在奇点、界面和临界点附近表现出幂律标度特性，然而标准的神经网络往往使主导指数保持隐式。我们引入了物理信息 Müntz-Szász 网络（MSN-PINN），这是一种将标度指数视为可训练参数的幂律基网络。该模型同时输出解及其标度结构。我们证明了可辨识性（即唯一恢复性），并表明在这些条件下，学习到的指数与真实指数之间的平方误差按 $O(|μ- α|^2)$ 缩放。在各项实验中，MSN-PINN 在噪声和稀疏采样下实现了 1-5% 误差的单指数恢复。它以 0.009% 的误差恢复了二维拉普拉斯方程的角点奇异性指数，与 Kondrat'ev (1967) 的经典结果相吻合，并以 0.03% 和 0.05% 的误差恢复了奇异泊松问题中由外力诱导的指数。在一个包含 40 种配置的楔形基准测试中，它达到了 100% 的成功率，平均误差为 0.022%。约束感知训练编码了诸如边界条件相容性等物理要求，并比朴素训练提高了三个数量级的精度。通过将神经网络的表达能力与渐近分析的可解释性相结合，MSN-PINN 产生的学习参数具有直接的物理意义。
