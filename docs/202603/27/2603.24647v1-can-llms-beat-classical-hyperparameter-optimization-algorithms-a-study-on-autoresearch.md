...
tldr: 本研究探讨了大型语言模型（LLM）在超参数优化（HPO）任务中相较于传统算法的表现。通过 autoresearch 平台，对比了 LLM 代理与 CMA-ES、TPE 等经典算法。研究发现，虽然在固定搜索空间内经典算法占优，但 LLM 在无约束代码编辑场景下表现出色。为此，作者提出了 Centaur 混合方法，将 CMA-ES 的内部状态与 LLM 结合。实验证明，即使是 0.8B 的小型 LLM 在混合模式下也能超越 27B 模型和纯经典算法，实现了领域知识与数值优化能力的互补。
selection_source: fresh_fetch
motivation: 旨在评估 LLM 代理在超参数优化中是否能超越传统算法，并解决 LLM 缺乏优化状态追踪及传统算法缺乏领域知识的问题。
method: 提出了 Centaur 混合优化框架，将 CMA-ES 的均值向量、步长和协方差矩阵等内部状态共享给 LLM 进行决策。
result: Centaur 混合方法在实验中取得了最佳性能，且 0.8B 版本的表现优于 27B 版本，证明了小型模型在混合优化中的有效性。
conclusion: 结合经典优化器的数值能力与 LLM 的领域知识是提升 HPO 效率的关键，且在混合模式下模型规模并非决定性因素。
---
