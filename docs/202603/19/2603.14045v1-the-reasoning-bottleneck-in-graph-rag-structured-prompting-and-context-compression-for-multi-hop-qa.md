...
## 摘要
Graph-RAG 系统通过将文档索引到知识图中实现了强大的多跳问答能力，但强大的检索能力并不保证能产生准确的答案。在三个多跳问答基准测试（HotpotQA、MuSiQue、2WikiMultiHopQA）上评估领先的 Graph-RAG 系统 KET-RAG 时，我们发现 77% 到 91% 的问题在检索到的上下文中包含标准答案，但准确率仅为 35% 到 78%，且 73% 到 84% 的错误归因于推理失败。我们提出了两种增强方案：(i) SPARQL 思维链（CoT）提示，它将问题分解为与实体关系上下文对齐的三元组模式查询；(ii) 图游走压缩，通过知识图遍历在不调用大语言模型（LLM）的情况下将上下文压缩约 60%。SPARQL CoT 将准确率提高了 2 到 14 个百分点；在较小模型上配合结构化提示时，图游走压缩平均可提升 6 个百分点。令人惊讶的是，我们证明了通过问题类型路由，一个经过全面增强的低成本开源 Llama-8B 模型在所有三个基准测试中，均以约 12 倍更低的成本达到或超过了未增强的 Llama-70B 基准。在 LightRAG 上的复现实验证实，我们的增强方案可以在不同的 Graph-RAG 系统间迁移。
