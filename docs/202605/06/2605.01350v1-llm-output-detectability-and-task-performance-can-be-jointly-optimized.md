...
title_zh: LLM 输出的可检测性与任务性能可以协同优化
...
tldr: 随着大语言模型的广泛应用，检测机器生成文本变得至关重要。传统水印技术虽可靠但常损害模型性能。本文提出PUPPET框架，通过强化学习微调LLM，同时优化文本的可检测性和下游任务表现。实验证明，PUPPET在长问答、摘要和写作任务中，不仅达到了与水印技术相当的可检测性，且在任务性能上表现更优，具有高效、泛化性强及抗改写攻击等优点。
selection_source: fresh_fetch
motivation: 旨在解决现有机器生成文本检测技术在提高可检测性的同时，往往会导致模型下游任务性能下降的问题。
method: 提出PUPPET框架，利用强化学习结合检测器似然得分和任务评估指标作为双重奖励函数来微调大语言模型。
result: 实验显示PUPPET在保持高检测率的同时，在多项任务上的表现优于传统水印方法，且训练高效并能抵抗改写攻击。
conclusion: 研究证明了通过联合优化可以同时提升LLM输出的可检测性与任务性能，为负责任的AI部署提供了高效且鲁棒的新方案。
---

## 摘要
检测机器生成的文本对于部署大语言模型（LLM）时的透明度和问责制至关重要。在各种检测方法中，水印技术在设计上是一种统计可靠的方法——它通过偏置 token 分布将可检测信号嵌入到 LLM 输出中。然而，据报道，添加水印的 LLM 在下游任务上的表现往往会有所下降。我们提出了 PUPPET，这是一个通过强化学习微调 LLM 的框架，旨在生成既更具可检测性又在下游任务中表现更好的文本。我们使用了两个奖励函数：一个输出机器类别概率的检测器，以及一个衡量特定任务指标的评估器。在长篇问答、摘要生成和论文写作方面的实验表明，使用 PUPPET 训练的 LLM 实现了与水印方法相当的高可检测性，同时在下游任务上的表现优于后者。分析表明，这种优化可以高效地进行，仅需几千个样本，在 1-2 个 GPU 小时内即可完成。此外，这些收益在跨领域任务、不同的 LLM 系列和模型规模上保持一致，甚至对改写攻击具有鲁棒性。
