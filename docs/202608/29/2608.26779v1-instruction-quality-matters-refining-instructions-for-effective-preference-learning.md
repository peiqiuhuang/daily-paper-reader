...
## 速览
**TLDR**：本研究揭示了指令质量是偏好学习中的关键瓶颈，低质量指令会限制响应质量并削弱偏好信号。作者提出了一种指令精炼流水线，通过奖励信号识别弱指令并利用LLM进行修订。实验表明，该方法在多种模型和基准测试中显著提升了对齐效果，证明了优化指令本身是增强LLM对齐性能的有效途径。 \
**Motivation**：低质量或模糊的指令会限制生成响应的质量分布，从而削弱偏好学习中信号的有效性。 \
**Method**：开发了一套指令精炼流水线，利用奖励信号筛选低质指令并基于LLM反馈进行自动修订。 \
**Result**：在离线和在线偏好学习实验中，精炼后的指令在多个基准测试上均取得了优于原始数据和传统策略的对齐效果。 \
**Conclusion**：指令质量是决定偏好信号信息量的核心因素，通过精炼指令可以显著提升大语言模型的对齐上限。
