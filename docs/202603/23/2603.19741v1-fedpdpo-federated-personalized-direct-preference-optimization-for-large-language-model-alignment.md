...
## 摘要
在联邦学习（FL）中将大语言模型（LLMs）与人类偏好对齐面临挑战，原因在于偏好数据具有去中心化、隐私敏感且高度非独立同分布（non-IID）的特性。直接偏好优化（DPO）为基于人类反馈的强化学习（RLHF）提供了一种高效的替代方案，但其在联邦学习中的直接应用在 non-IID 数据下会遭受严重的性能退化，且隐式奖励的泛化能力有限。为了弥补这一差距，我们提出了 FedPDPO（联邦个性化直接偏好优化），这是一个用于大语言模型偏好对齐的个性化联邦框架。该框架采用参数高效微调架构，每个客户端维护一个冻结的预训练大语言模型主干，并辅以低秩自适应（LoRA）适配器，从而实现通信高效的聚合。为了解决 non-IID 异构性，我们设计了：（1）全局共享的 LoRA 适配器与个性化的客户端特定大语言模型头部；（2）一种带有客户端特定显式奖励头部的个性化 DPO 训练策略，以补充隐式奖励并进一步缓解 non-IID 异构性；以及（3）一个瓶颈适配器（bottleneck adapter）来平衡全局和局部特征。我们提供了理论分析，建立了概率基础和合理性。在多个偏好数据集上的广泛实验证明了该方法具有最先进的性能，在联邦域内和跨域设置中实现了高达 4.80% 的平均准确率提升。
