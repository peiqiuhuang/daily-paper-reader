...
date: 2026-09-09
pdf: "https://arxiv.org/pdf/2609.09567v2"
tags: ["query:bopl"]
score: 6.0
evidence: 结构化提示边界的任务条件调节
tldr: 针对大型产品目录中因重复和单位不匹配导致的质量问题，本文提出了一种位置任务调节（PTC）方法。通过将复杂的缺陷检测任务分解为子任务，并利用PTC在结构化提示边界强化任务身份，该方法在保持高性能的同时显著降低了计算成本，实现了大规模产品家族的高效缺陷检测。
selection_source: fresh_fetch
motivation: 大型产品目录中存在的产品信息不一致问题严重影响用户体验，而现有大模型在处理长文本时分类质量下降且成本高昂。
method: 将检测任务分解为聚焦的子任务，并引入位置任务调节（PTC）技术，通过在结构化提示边界强化任务身份，将复杂能力蒸馏至小型模型。
result: "PTC方法在五个模型和两个架构系列上表现优异，F1分数从52%提升至87%，且成本降低高达98%。"
conclusion: PTC提供了一种高效、可扩展的解决方案，已成功应用于处理千万级产品家族的实际生产环境。
---

## 摘要
大型产品目录中的产品系列常存在重复项和单位不匹配等不一致问题，严重影响客户体验。检测这些问题需要在冗长的产品列表中对多种错误类型进行推理，而大语言模型（LLM）的分类质量往往受限于长上下文能力。我们通过将检测任务分解为聚焦的子任务来降低上下文复杂度并隔离错误类型，从而将 F1 分数从 52% 提升至 87%。为实现可扩展部署，我们引入了位置任务调节（Positional Task Conditioning, PTC），通过在结构化提示边界强化任务标识，将此能力提炼至单一小型模型中。PTC 在五个模型和两个架构系列上的表现均优于基于推理的蒸馏方法，在成本降低高达 98% 的情况下，F1 分数与前沿模型差距在 1.79% 以内。我们的系统已在多个国家部署，处理超过 1000 万个产品系列。
