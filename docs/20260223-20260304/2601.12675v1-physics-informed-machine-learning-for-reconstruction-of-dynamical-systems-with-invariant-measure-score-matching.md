...
tags: ["query:sr"]
score: 7.0
evidence: 从无标签点云数据中重建动力系统
tldr: 本文提出了一种名为PINN-IMSM的无网格框架，用于从捕获不变测度的无标签点云数据中重建动力系统。该方法通过去噪评分匹配直接从数据中估计得分函数，并将其嵌入物理信息神经网络（PINN）中，以求解稳态Fokker-Planck方程。通过将重建问题转化为PDE约束的最小能量速度场优化问题，该框架有效解决了高维逆问题的病态性，并在多维混沌系统实验中展现了卓越的重建精度。
selection_source: fresh_fetch
motivation: 旨在解决从无标签点云数据中重建高维动力系统时，传统方法面临的维度灾难和密度估计困难。
method: 结合去噪评分匹配与物理信息神经网络，通过求解基于得分函数的稳态Fokker-Planck方程来重构漂移速度场。
result: 在范德波尔振子、混沌洛伦兹系统等多个动力系统实验中，该方法成功实现了高达五维空间的不变测度恢复和动力学行为重建。
conclusion: PINN-IMSM框架为高维动力系统的逆向建模提供了一种高效且具有理论保证的无网格解决方案。
---

## 摘要
本文开发了一种名为“结合不变测度得分匹配的物理信息神经网络”（PINN-IMSM）的新型无网格框架，用于从捕获系统不变测度的无标签点云数据中重构动力系统。不变密度满足稳态 Fokker-Planck (FP) 方程。我们根据得分函数（对数密度的梯度）重新表述该方程，并通过去噪得分匹配直接从数据中估计得分函数，从而绕过了显式的密度估计。随后，将学习到的得分嵌入到物理信息神经网络（PINN）中，以在生成的基于得分的 FP 方程下重构漂移速度场。PINN 的无网格特性使该框架能够扩展到更高维度，避免了基于网格的方法中固有的维度灾难。为了解决高维反问题的病态性，我们将该问题重新表述为一个寻求最小能量速度场的 PDE 约束优化问题。在适当的条件下，我们证明了该问题存在唯一解，且该解连续依赖于得分函数。该约束公式采用随机增广拉格朗日方法求解。对代表性动力系统（包括 Van der Pol 振子、非谐波陷阱中的主动游泳者以及混沌 Lorenz-63 和 Lorenz-96 系统）的数值实验表明，PINN-IMSM 能够准确恢复不变测度，并在高达五维的问题中重构出真实的动力学行为。
