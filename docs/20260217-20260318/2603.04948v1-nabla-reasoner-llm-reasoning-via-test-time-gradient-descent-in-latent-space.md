...
tags: ["query:bopl"]
score: 8.0
evidence: 针对Token Logits的可微优化以改进策略
tldr: 针对大语言模型推理阶段计算扩展效率低的问题，本文提出 $\nabla$-Reasoner 框架。该框架通过“可微分文本优化”（DTO）在解码过程中利用梯度下降优化 Token Logits，结合奖励模型信号实时修正生成策略。相比传统的离散搜索，该方法实现了从零阶搜索到一阶优化的范式转变，在数学推理任务上显著提升了准确率并降低了计算开销。
selection_source: fresh_fetch
motivation: 现有的推理阶段扩展方法主要依赖低效的离散搜索或试错提示，难以实现对在线策略的高效优化。
method: 提出可微分文本优化（DTO）机制，在解码循环中利用 LLM 似然和奖励模型的梯度信号对文本表示进行迭代优化。
result: "在挑战性数学推理基准上实现了超过 20% 的准确率提升，同时比强基准方法减少了 10-40% 的模型调用次数。"
conclusion: 该研究证明了在推理阶段引入一阶梯度优化是提升 LLM 推理能力的一种高效且低成本的新范式。
---
