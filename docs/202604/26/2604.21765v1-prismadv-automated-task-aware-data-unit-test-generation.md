...
## 摘要
数据是现代企业的核心资源，数据验证对于确保下游应用的可靠性至关重要。然而，现有的自动化数据单元测试框架在很大程度上是任务无关的：它们在验证数据集时，并未考虑消耗该数据的代码语义和需求。我们提出了 PrismaDV，这是一个复合型人工智能系统，它结合数据集概况（dataset profiles）分析下游任务代码，以识别数据访问模式、推断隐式数据假设，并生成任务感知的可执行数据单元测试。为了使数据单元测试能够随时间进一步适应特定的数据集和下游任务，我们提出了“任务自适应的选择性信息反馈”（SIFTA），这是一个提示词优化框架，利用了数据单元测试和下游任务执行中稀缺的结果反馈。我们在涵盖 5 个数据集、60 个任务的两个新基准测试上评估了 PrismaDV。结果表明，在生成反映数据错误端到端影响的单元测试方面，它始终优于任务无关和任务感知的基准方法。此外，我们证明了通过 SIFTA，我们可以为 PrismaDV 的模块自动学习提示词，其性能优于人工编写或由通用提示词优化器生成的提示词。我们公开了基准测试和原型实现。
