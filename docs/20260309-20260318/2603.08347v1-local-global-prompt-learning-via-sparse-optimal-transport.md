...
conclusion: 无投影的局部对齐能够保留CLIP原始流形的几何特性，在优化分类性能的同时显著提升了模型的分布外检测能力。
---

## 摘要
诸如 CLIP 之类的视觉语言模型（VLM）的少样本适配通常依赖于学习与全局图像嵌入相匹配的文本提示。最近的研究通过引入局部图文对齐来捕捉细粒度视觉线索，从而扩展了这一范式，但这些方法通常为每个提示独立选择局部区域，导致局部特征使用冗余以及提示重叠。我们提出了 SOT-GLP，它引入了共享的稀疏补丁（patch）支撑和平衡最优传输分配，在保持全局对齐的同时，显式地在类别特定的局部提示之间划分显著视觉区域。我们的方法学习共享的全局提示和类别特定的局部提示。全局分支维持标准的图文匹配，以实现稳健的类别级对齐。局部分支利用 V-V 注意力构建类别条件的稀疏补丁集，并通过平衡熵最优传输将其与多个类别特定的提示对齐，从而产生补丁的软划分，防止提示重叠和崩溃。我们在两个互补的目标上评估了我们的方法：(i) 在 11 个标准基准测试上的少样本分类准确率，以及 (ii) 分布外（OOD）检测。在使用 16-shot ViT-B/16 的标准 11 数据集基准测试中，SOT-GLP 达到了 85.1% 的平均准确率，优于之前的提示学习方法。我们在提示学习中发现了一个明显的准确性-鲁棒性权衡：虽然可学习的投影优化了分布内拟合，但它们改变了基础特征空间。我们证明了无投影的局部对齐保留了 CLIP 流形的原生几何结构，从而产生了超越完全适配模型的先进 OOD 检测性能（94.2% AUC）。代码实现见：https://github.com/Deniz2304988/SOT-GLP
