...
title_zh: 面向鲁棒两阶段以物体为中心的视觉推理的弱监督概念瓶颈学习
...
tldr: 本研究针对神经符号架构中视觉谓词标注成本高的问题，提出了动态正交概念瓶颈（D-OCB）框架。该框架基于对象中心槽位VAE，通过动态学习超参数分配和惩罚概念子空间相关性，在极弱监督下提取人类可理解的符号谓词。其核心贡献在于引入动态维度分配机制，防止表示崩溃并提升概念准确性，在极低标注预算下实现了卓越的推理性能。
selection_source: fresh_fetch
motivation: 传统的神经符号架构在将高层谓词锚定到视觉帧时，通常需要昂贵且难以获取的人工标注。
method: 提出了D-OCB框架，结合了动态超参数学习、正交概念子空间惩罚以及防止表示崩溃的动态维度分配机制。
result: 实验证明该框架在极少标注预算下实现了高度的概念对齐和推理准确率，性能达到或超过了端到端范式。
conclusion: D-OCB通过自适应机制有效解决了弱监督学习中的不稳定性，为鲁棒的对象中心视觉推理提供了高效的解决方案。
---
