...
tags: ["query:bopl"]
score: 8.0
evidence: 用于多模态估计的软提示学习
tldr: 本研究针对自然场景下的效价-唤醒度（VA）估计，提出了一种距离感知软提示学习框架。为解决CLIP等模型在连续回归任务中受限于离散提示的问题，该方法将VA空间划分为3x3网格，利用高斯核根据欧氏距离生成软标签，以捕捉细粒度的情感转换。结合CLIP图像编码器、AST音频特征及GRU时序建模，通过分层融合机制实现多模态对齐，在Aff-Wild2数据集上取得了竞争性性能。
selection_source: fresh_fetch
motivation: 旨在克服预训练视觉语言模型在处理连续情感回归任务时，因文本提示的离散性而导致的语义对齐局限。
method: 引入距离感知软提示学习，通过VA空间分区和高斯核软标签，结合CLIP、AST及分层注意力融合机制进行多模态建模。
result: 在Aff-Wild2数据集上的实验结果表明，该方法显著提高了VA估计的准确性，在复杂野外环境下表现出色。
conclusion: 语义引导的软提示学习能有效弥合离散语义与连续情感维度间的鸿沟，是提升多模态情感识别的有效途径。
---
