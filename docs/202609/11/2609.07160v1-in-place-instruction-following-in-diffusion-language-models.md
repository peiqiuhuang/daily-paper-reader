...
tldr: 本文研究了扩散语言模型（dLLMs）在“原地提示”（IPP）任务中的表现，即在生成文本的特定位置执行指令。作者提出了IIF-Bench基准测试，并发现现有模型在处理约束时存在优先级不足的问题。为此，作者开发了GRAFT训练框架，通过约束感知监督微调和偏好优化，显著提升了模型在字面、风格及语篇约束下的指令遵循能力，同时保持了通用生成水平。
selection_source: fresh_fetch
motivation: 现有扩散语言模型在处理特定位置的指令约束时往往表现不佳，缺乏有效的评估基准和针对性优化。
method: 提出了IIF-Bench基准测试，并开发了名为GRAFT的后训练框架，结合了约束感知监督微调与偏好优化技术。
result: 在四种代表性模型上，GRAFT将平均IIF得分从57.75提升至73.10，在字面和语篇约束任务上取得了显著进步。
conclusion: GRAFT框架有效增强了扩散语言模型在原地指令遵循任务中的精确度，且未损害其基础生成能力。
---
