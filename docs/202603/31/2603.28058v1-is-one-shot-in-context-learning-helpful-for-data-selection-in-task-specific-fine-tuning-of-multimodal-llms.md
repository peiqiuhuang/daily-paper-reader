...
tags: ["query:bopl"]
score: 6.0
evidence: 用于微调数据选择的单样本上下文学习
tldr: 针对多模态大模型（MLLM）在特定任务微调中面临的数据规模与计算开销平衡问题，本文提出了一种无需训练的数据选择流水线 CLIPPER。该方法利用单样本上下文学习（ICL）来探测模型对不同样本组合的响应，通过识别镜像原始数据集困惑度分布的核心子集，在保持数据多样性的同时保留关键样本。实验证明，CLIPPER 在显著降低训练成本的同时，能达到全量微调的性能水平。
selection_source: fresh_fetch
motivation: 现有的数据选择方法依赖额外评分模型或启发式聚类，难以兼顾数据重要性、多样性以及样本间的相互影响。
method: 提出 CLIPPER 流水线，利用上下文学习探测模型响应，并筛选出能反映原始数据集困惑度分布的核心子集。
result: "在多个模型和数据集上，CLIPPER 显著提升了数据效率并缩短了训练时间，例如在 ScienceQA 上减少了 37% 的训练时间。"
conclusion: 单样本上下文学习是多模态大模型特定任务微调中进行高效、低成本数据选择的有效手段。
---
