...
score: 9.0
evidence: 自动提示词优化框架
tldr: 针对现有自动提示优化方法难以跨任务泛化且无法积累经验的问题，本文提出MemAPO框架。该框架通过双重记忆机制，将成功的推理轨迹提炼为策略模板，并将错误生成组织为失败模式。通过检索相关经验并结合自我反思与记忆编辑，MemAPO实现了提示优化的持续进化，在提升性能的同时显著降低了优化成本。
selection_source: fresh_fetch
motivation: 现有的提示优化方法通常局限于特定任务，缺乏跨异构查询的泛化能力和可重用的知识积累机制。
method: 提出一种双重记忆框架，通过提炼成功策略模板和总结错误模式，并利用自我反思与记忆编辑实现提示的动态进化。
result: 实验表明MemAPO在多个基准测试中均优于现有基准方法，并显著降低了优化过程中的计算开销。
conclusion: 该研究证明了通过记忆驱动的经验积累，可以实现更具泛化性、高效且能够自我进化的自动提示优化系统。
---

## 摘要
自动提示词优化是将大语言模型（LLMs）适配到下游任务的一种极具前景的方法，然而现有方法通常是针对固定任务搜索特定的提示词。这种范式限制了在异构查询之间的泛化能力，并阻碍了模型随时间积累可重用的提示词知识。在本文中，我们提出了 MemAPO，这是一个记忆驱动的框架，它将提示词优化重新定义为可泛化且自进化的经验积累过程。MemAPO 维持一种双重记忆机制，将成功的推理轨迹提炼为可重用的策略模板，同时将错误的生成组织成捕获循环失败模式的结构化错误模式。给定一个新的提示词，该框架会检索相关的策略和错误模式，以构建既能促进有效推理又能避免已知错误的提示词。通过迭代的自我反思和记忆编辑，MemAPO 持续更新其记忆，使提示词优化能够随时间不断改进，而不是针对每个任务都从头开始。在多种基准测试上的实验表明，MemAPO 一贯优于代表性的提示词优化基准方法，同时显著降低了优化成本。
