...
tags: ["query:bopl"]
score: 8.0
evidence: 提示词与响应的联合评估框架
tldr: 针对大语言模型提示词评估仅关注正确性而缺乏解释性的问题，本文提出PEEM框架。该框架通过LLM对提示词（3个维度）和回答（6个维度）进行联合评估，输出1-5分的分值及解释性理由。实验表明，PEEM与传统准确率指标高度一致，且能有效捕捉语言层面的失效模式。利用PEEM的反馈进行零样本提示词重写，可显著提升下游任务表现，为系统化诊断和优化LLM交互提供了有力工具。
selection_source: fresh_fetch
motivation: 现有的评估方法主要关注回答的正确性，无法解释提示词为何成功或失败，也难以提供可操作的优化指导。
method: 提出PEEM框架，定义了涵盖提示词和回答质量的9个评估维度，并利用LLM生成标量评分和基于准则的自然语言解释。
result: PEEM在准确性维度上与传统指标高度相关（相关系数约0.97），且基于其反馈的提示词优化效果优于监督学习和强化学习基准。
conclusion: PEEM建立了一种可解释且可复现的评估协议，将提示词表述与回答行为联系起来，实现了LLM交互的系统化诊断与优化。
---

## 摘要
提示词设计是大语言模型（LLMs）的主要控制接口，然而标准的评估方法在很大程度上将性能简化为回答的正确性，掩盖了提示词成功或失败的原因，且几乎无法提供可操作的指导。我们提出了 PEEM（提示工程评估指标），这是一个用于对提示词和响应进行联合且可解释评估的统一框架。PEEM 定义了一个包含 9 个维度的结构化评分标准：3 个提示词标准（清晰度/结构、语言质量、公平性）和 6 个响应标准（准确性、连贯性、相关性、客观性、清晰度、简洁性），并使用基于 LLM 的评估器输出 (i) 1-5 分李克特量表（Likert scale）的标量分数，以及 (ii) 基于评分标准的特定准则自然语言原理解释。在 7 个基准测试和 5 个任务模型中，PEEM 的准确性维度与传统准确率高度一致，同时保留了模型排名（综合 Spearman rho 约为 0.97，Pearson r 约为 0.94，p < 0.001）。一项涉及四个模型的多评估器研究显示了一致的相对判断（两两 rho = 0.68-0.85），支持了与评估器无关的部署。除了对齐性之外，PEEM 还能捕捉互补的语言失败模式，并在提示词扰动下保持信息量：提示词质量趋势在迭代重写中追踪下游准确率，语义对抗操纵会导致明显的分数下降，而保持原意的改写则表现出高度的稳定性（鲁棒率约为 76.7-80.6%）。最后，仅使用 PEEM 分数和原理解释作为反馈，零样本提示词重写循环将下游准确率提高了多达 11.7 个百分点，优于基于监督学习和强化学习（RL）的提示词优化基准。总之，PEEM 提供了一个可复现的、准则驱动的协议，将提示词构建与响应行为联系起来，并实现了对 LLM 交互的系统性诊断和优化。
