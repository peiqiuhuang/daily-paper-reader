...
tldr: 本研究探讨了多模态大语言模型（MLLM）在处理图像内嵌指令时的性能表现。通过引入“可视化任务语义（VTS）”干预实验，研究发现将问题从文本移动到图像像素中会导致模型准确率平均下降17.8个百分点，揭示了模型在处理不同通道指令时存在语义鸿沟。为此，作者提出了提示区域对齐（Prompt-Region Grounding）方法，通过将图像问题区域与文本语义对齐并进行掩码恢复训练，显著提升了模型对视觉指令的推理能力，且无需推理时OCR。
selection_source: fresh_fetch
motivation: 旨在研究多模态模型在面对嵌入图像像素中的任务指令时，是否能保持与处理文本指令相同的推理效能。
method: 提出提示区域对齐方法，通过将图像中的问题区域与打字语义对齐，并从掩码视图中恢复其清晰表示来增强模型理解。
result: 该方法在四个基准测试中将VTS准确率从58.0提升至66.3，同时保留了原始接口的性能，且推理时无需OCR或区域元数据。
conclusion: 阅读图像中的任务文本与将其作为推理指令进行对齐是两种不同的能力，模型需要专门的对齐机制来弥合语义通道差距。
---
