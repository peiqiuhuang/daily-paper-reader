...
title_zh: 大推理模型中提示词诱导的浪费：一项针对编程智能体的预注册双框架基准测试
...
tldr: 本研究针对大语言模型在编程代理任务中的推理成本浪费问题，通过预注册基准测试，在6个模型和2个框架上进行了4643次实验。研究发现，提示词表述（如要求深度思考或比较多种方案）会显著增加推理成本（最高达7.4倍）却不提升成功率，而框架设计对成本的影响甚至更大。该研究揭示了提示词与架构对AI代理经济效率的重大影响，为优化推理成本提供了实证依据。
selection_source: fresh_fetch
motivation: 旨在系统测量提示词表述对大语言模型编程代理推理成本的因果影响，揭示在不提升性能情况下的资源浪费现象。
method: 采用预注册的基准测试方法，在6个推理模型和2个代理框架下，针对24个编程任务进行了超过4600次运行实验。
result: 发现特定提示词会使推理成本激增2.4至7.4倍而不提高正确率，且不同框架间的成本差异最高可达30倍。
conclusion: 提示词表述和框架设计显著影响代理运行成本，通过优化指令和架构可在不损失成功率的前提下大幅降低开销。
---

## 摘要
作为编程智能体使用的大推理模型会因深思熟虑、工具调用和重复的智能体轮次而产生开销，但提示词措辞对这类支出的因果影响尚未得到系统测量。我们展示了一项预注册的基准测试，涵盖了六个大推理模型、两个真实的智能体框架以及 24 个带有隐藏评估器的确定性编程任务。在包括筛选、压力测试、留存、复制和跨供应商研究在内的 4,643 次有效运行中，我们发现提示词的表述可能会使推理成本成倍增加，而不会提高正确性。要求模型开发并比较多种方法是最持续浪费的指令，在所有模型中使推理 Token 增加了 2.4-7.4 倍。通用的“深入思考”提示也会使深思熟虑增加 1.6-2.2 倍，而指定范围、验收标准和停止条件的有界效率模板则是成本中性的，并且可以将推理减半。框架的选择更为重要：在 Claude Code 下，相同的“模型-任务-提示词”三元组在每次成功时的成本比在 pi 下高出 5-30 倍，这主要是由于更大的静态前缀和更多的轮次。误导性的架构提示比无关的散文成本高得多，而供应商端的缓存虽然降低了计费成本但并未改变行为，因此绝不能将其视为效率。在 Kimi-K3 和 Claude Sonnet 5 上的复制实验保留了主要效应方向，同时揭示了模型对思考和确定性提示的特定敏感性。总的来说，提示词措辞和框架设计实质性地影响了智能体成本，且通常不会带来任务成功率的提升。
