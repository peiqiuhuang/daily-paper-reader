...
tags: ["query:sr"]
score: 6.0
evidence: 用于解决线性算子特征值问题的物理信息回归
tldr: 本研究针对物理信息高斯过程回归（PI-GPR）在求解线性算子特征值问题时面临的零源项导致预测均值平凡化和边际似然退化的问题，提出了一种基于传递函数型指标的新方法。通过构建后验协方差指标，研究证明了只有当参数对应算子特征值时，后验协方差才非平凡，且后验样本均位于特征空间内。该方法在多种线性和非线性特征值问题中展现了有效性。
selection_source: fresh_fetch
motivation: 传统的物理信息高斯过程在处理特征值问题时，由于源项为零，会导致预测结果趋于平凡解且似然函数退化。
method: 借鉴系统辨识思想，利用PI-GP后验构建传递函数型指标，通过后验协方差的非平凡性来识别特征值和特征函数。
result: 数值实验表明，该方法能准确识别线性及非线性算子的特征值，且生成的后验样本严格落在对应的特征空间内。
conclusion: 该研究为利用高斯过程解决算子特征值问题提供了一种有效的非平凡解构建方案，扩展了PI-GPR的应用范围。
---
